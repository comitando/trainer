...
final class CreateAccountRepositoryTests: XCTestCase {
...
private extension CreateAccountRepositoryTests {
    
    func make() throws -> (sut: CreateAccountRepository, database: UserAccountData) {
        let database = try UserAccountData(useInMemoryStore: true)
        let sut = CreateAccountRepository(database: database)
        return (sut, database)
    }
}
